...
For all the figures, we used the data table downloaded from the European Social Survey Website.  Each entry in the table gives us the survey result for one participant in the UK. The variable representing the income is as income, which contains the data from variable hinctnt or hinctnta depending on the survey version. The data is visually demonstrated by a scatter plot, bar plot, and scatter graph.

The raw data contain 26 variables with 19830 rows. After cleaning and restructure the data, we choose to focus of the data of 2006 and 2010.

For the survey data of 2006, we have 8 variables with 1858 rows. the variables including lrscale, gndr, agea, edulvla, plnftr, income and year. The variable scale represents the participants’ political stand by defining “left” and “right.” The variable gndr represents the participant’s gender, which is male or female. In this survey, the other genders were not taken as options. The variable agea represents the participant’s age. The variable edulvla represents the participant’s highest education level. The variable plnftr represents the participant’s planning habit scale, where 0 means “I plan for my future as much as possible” and 10 means “I just take each day as it comes”.

For the survey data of 2010, we have 8 variables with 1858 rows—the variables including lrscale, gndr, agea, edulvla, plnftr, income and year. The variable lrscale represents the participants’ political stand by the definition of “left” and “right.” The variable gndr represents the participant’s gender, which is male or female. In this survey, the other genders were not taken as options. The variable agea represents the participant’s age. The variable edulvla represents the participant’s highest education level. The variable wrywprb represents the level of participants’ worry about work problems when they are not working, where 0 and 5 correspond to never and always. 

There are some missing values for all the variables in both data sets. However, we cannot remove all of them at once to get a complete data set. Thus, we will only filter out some data if the variable is used in our analysis. 
\newpage
...
```{r figure-1, echo=FALSE,fig.height=4}
...
```{r figure2, echo=FALSE, fig.height=4}
